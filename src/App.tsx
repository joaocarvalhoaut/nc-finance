...
export default function App() {
  ...
  const handleAIExtract = async () => {
    if (!importText.trim()) {
      setExtractionAlert("Escreva ou cole informações de cobrança no editor antes de prosseguir.");
      return;
    }
    ...
    try {
      const response = await fetch("/api/gemini/extract", {
        ...
        body: JSON.stringify({ 
          ...
          category: importCategory === "vencidos" ? "Vencidos" : importCategory === "a_vencer" ? "A vencer" : "Liquidação"
        })
      });
      ...
      if (data.debtors && Array.isArray(data.debtors)) {
        const parsedList = data.debtors.map((item: ExtractedDebtorCandidate, index: number) => ({
          ...
          client: item.client || "Cliente Desconhecido",
          supplier: item.supplier || "NC Finance",
          document: item.document || `9982-${index}`,
          dueDate: item.dueDate || "11/03/2026",
          value: Number(item.value) || 1200.00,
          phone: item.phone || "",
        ...
        if (data.warning) {
          setExtractionAlert(data.warning);
        }
      } else {
        setExtractionAlert("Nenhum dado financeiro puder ser extraído de forma estruturada. Verifique o padrão textual.");
      }
    } catch (err) {
      setExtractionAlert("Desculpe, falha ao conectar no extrator Gemini AI. Usando simulação inteligente integrada.");
    } finally {
  ...
  const handleSendMessage = async () => {
    ...
    // Pré-checagem visual (backend re-valida tudo — isso só evita round-trip desnecessário)
    if (!canSendCharge) {
      setMessageFeedback({
        success: false,
                            ...
                            <>
                              ...
                              {operationalMetrics.recentErrors.length > 0 && (
                                <div className="pt-1 border-t border-zinc-800/60">
                                  <p className="text-[10px] text-rose-400 font-semibold mb-1">Últimos erros:</p>
                    ...
                    <div className="lg:col-span-5 bg-zinc-900/40 border border-zinc-900 p-6 rounded-3xl space-y-4 shadow-xl">
                      ...
                      <div className="p-4 rounded-2xl bg-zinc-950 border-2 border-dashed border-zinc-800 hover:border-emerald-500/30 transition-all text-center space-y-2 relative group">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-zinc-600">Simulação de arquivo local de cobranças</p>
                        </div>
                        <input 
                          type="file" 
                          accept=".pdf,.txt,.xlsx,.csv"
                          onChange={(e) => {
                            // Automatically insert mock report based on user sample values upon uploading file
                            setImportText(`RELATORIO DE FATURAMENTO DIARIO - COMPLIANCE NC
Documento emitido para Carlos Eduardo Neves ref. boleto faturado com id 4241-2, emissor NC Empreendimentos no valor total original de $ 2248,60 para vencer em 11/03/2026, tel do sac 5577999887720. Consta também inadimplência de Maria Silva Bastos cnpj 22.339.421/0001-90, id boleto 8891-B no valor base de 540,00 reais vcto 25/08/2026, fone 5577999112233. Por favor processar.`);
                            alert("Arquivo carregado e inserido no editor para extração de dados!");
                          }}
...
1. Carlos Eduardo Neves - Boleto 4241-2 - Vencimento 11/03/2026 - Valor R$ 2.248,60 - tel 5577999887720
2. Mariana Silva Bastos - Boleto 8891-B - Vencimento 25/08/2026 - Valor R$ 540,00 - tel 5577999112233`)}
                          ...
                          >
                            Preset 1: Dados do boleto original (2 clientes)
                          </button>
                          <button
                            type="button"
                            onClick={() => setImportText(`RELATÓRIO FINANCEIRO BRUTO DE PENDÊNCIAS
Devedor: Julio César de Mello, boleto 2104-E emitido por NC Empreendimentos para fechar no valor de R$ 12500,00, data vencimento 10/05/2026. Telefone de suporte: 5511999445566, por favor fazer follow-up.`)}
                            className="w-full text-left p-2.5 rounded-lg bg-zinc-950 hover:bg-zinc-900 border border-zinc-900 text-[11px] text-zinc-400 block truncate"
                          >
                            Preset 2: Notificação de cobrança singular estruturada
                          </button>
                        </div>
                      </div>
              ...
              {/* ──────────────────────────────────────────────────────── */}
              {/* TAB: STATUS & COBRANÇA (WP, Z-API, Drive pairing) */}
              {/* ──────────────────────────────────────────────────────── */}
              {currentTab === "cobranca" && (
                <div className="space-y-8">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    {/* GOOGLE DRIVE REAL — PDF matching */}
                    <div className="lg:col-span-6 bg-zinc-900/40 border border-zinc-950 p-5 rounded-3xl space-y-4 shadow-xl flex flex-col justify-between">
                      <div className="space-y-3">
                        <div className="space-y-1">
                          <h4 className="text-sm font-bold text-white flex items-center gap-2">
                    ...
                    <div className="lg:col-span-7 bg-zinc-900/40 border border-zinc-900 p-6 rounded-3xl space-y-5 shadow-xl flex flex-col justify-between">
                      {selectedDebtorForMessage ? (
                        <>
                          ...
                          {/* Drive match display — real data from debtor */}
                          <div className="p-3.5 rounded-2xl bg-zinc-950 border border-zinc-850 space-y-1.5 text-xs text-zinc-400">
                            ...
                            {selectedDebtorForMessage?.driveFileId ? (
                              ...
                              <span className="text-zinc-500 text-xs italic">
                                Nenhum PDF pareado — clique em "Localizar PDFs no Drive" na aba de Cobrança.
                              </span>
                            )}
              ...
              {/* ──────────────────────────────────────────────────────── */}
              {/* TAB: HISTÓRICO DE COBRANÇAS (Z-API logs tracking) */}
              {/* ──────────────────────────────────────────────────────── */}
                                ...
                                <React.Fragment key={log.id}>
                                  <tr className="hover:bg-zinc-900/25 transition-colors">
                                    ...
                                    <td className="px-5 py-4">
                                      ...
                                      <div className="text-[10px] text-zinc-500 font-mono">D: {log.document} • Tél: {log.phone}</div>
                                    </td>
                                    <td className="px-5 py-4">
                                      <div className="flex items-center gap-2">
              ...
              {/* ──────────────────────────────────────────────────────── */}
              {/* TAB: AUTOMAÇÕES (scheduler + dispatch worker) */}
              {/* ──────────────────────────────────────────────────────── */}
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex flex-wrap gap-x-4 mt-1.5 text-[11px] text-zinc-500 font-mono">
                              {rule.sendWindowStart && rule.sendWindowEnd && (
                                <span>🕐 {rule.sendWindowStart}–{rule.sendWindowEnd}</span>
                              )}
                              {rule.maxDailySends != null && (
                                <span>📨 máx {rule.maxDailySends}/dia</span>
                              )}
                              {rule.lastRunAt && (
